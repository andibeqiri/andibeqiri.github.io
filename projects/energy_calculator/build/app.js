...
function addAppliance(e) {
	var applianceName = e.target.elements[0].value,
	...
	if (applianceName === "" || powerUsage === "" || hoursDay === "" || costWatt === "") {
		// Display error
		showAlert("Please fill in all the fields", "error");
	} else {
		var day = 24,
		    month = 30.42,
		    year = 12; // 24hour, 30days, 12 months
		var costDay = calculatePowerCost(powerUsage, hoursDay, costWatt);
		if (costDay === -1) {
			showAlert('An error occurred', 'error');
		} else {
			var costMonth = parseFloat((costDay * month).toFixed(4));
			var costYear = parseFloat((costMonth * year).toFixed(4));
	...
	e.target.elements[0].value = "", e.target.elements[1].value = "", e.target.elements[2].value = "", e.target.elements[3].value = "", e.preventDefault();
}
...
function showAlert(msg, className) {
	...
	var form = document.querySelector('#title-id');
	// Insert alert
